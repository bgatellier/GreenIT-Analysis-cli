 ...
 **/
function computeEcoIndex(dom: number, req: number, size: number): number {
...
function computeQuantile(quantiles: number[], value: number): number {
    for (let i = 1; i < quantiles.length; i++) {
        const currentQuantile = quantiles[i] as number
        if (value < currentQuantile) {
            const previousQuantile = quantiles[i - 1] as number
            return i + (value - previousQuantile) / (currentQuantile - previousQuantile)
        };
    }
...
function computeGreenhouseGasesEmissionfromEcoIndex(ecoIndex: number): number {
    return Math.round(100 * (2 + (2 * (50 - ecoIndex)) / 100)) / 100;
}

function computeWaterConsumptionfromEcoIndex(ecoIndex: number): number {
